...
function showAll(cardsArray) {
  cardsArray.forEach(function(card) {
    $(".displayCard").append('<div class="flip-container" ontouchstart="this.classList.toggle("hover");"><div class="flipper"><div class="front" id ="' + card.number + '"><p id ="' + card.number + '">' + card.term + '</p></div><div class="back" id ="' + card.number + '">' + '<p id ="' + card.number + '">' + card.definition + '</p></div></div></div>');
  });
};
...
function makeCards() {
  ...
  cardsArray.push(newCard);
  newCard = new Card("computerScience", "Ruby", "Switch statement", "Acts like a big if / else if / else chain. Checks a value against a list of cases, and executes the first case that is true. If it does not find a match, it attempts the default case designated by an else statement. If there is not a default case, then it exits the statement.");
  cardsArray.push(newCard);
  ...
  cardsArray.push(newCard);
  newCard = new Card("computerScience", "C Sharp", "constructor", "Same as a class. Constructors can take parameters. When not written by coder then are automatically generated by Visual Studio compiler.");
  cardsArray.push(newCard);
  newCard = new Card("computerScience", "C Sharp", "constant", "Field whose value can never change. Declared with 'const' keyword.");
  cardsArray.push(newCard);
  newCard = new Card("computerScience", "Dot Net", "CLR", "common language runtime - a virtual execution system");
  cardsArray.push(newCard);
  newCard = new Card("computerScience", "Dot Net", "assembly", "An assembly may include types such as interfaces, classes, structures, enumerations, and delegates. Assemblies in a project's bin folder are sometimes referred to as binaries.");
  cardsArray.push(newCard);
  newCard = new Card("computerScience", "Dot Net", "TFM", "Target framework moniker. A standardized token format for specifying the target framework of a .NET app or library.");
  cardsArray.push(newCard);
